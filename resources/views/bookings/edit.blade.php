                    ...
                    <form action="{{ route('bookings.update', $booking) }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="client_name" name="client_name" type="text" class="mt-1 block w-full" :value="old('client_name', $booking->client_name)" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('client_name')" />
                                </div>

                                <div>
                                    <x-input-label for="client_email" :value="__('Client Email')" />
                                    <x-text-input id="client_email" name="client_email" type="email" class="mt-1 block w-full" :value="old('client_email', $booking->client_email)" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('client_email')" />
                                </div>

                                <div>
                                    <x-input-label for="client_phone" :value="__('Client Phone')" />
                                    <x-text-input id="client_phone" name="client_phone" type="text" class="mt-1 block w-full" :value="old('client_phone', $booking->client_phone)" required />
                                ...
                                <div>
                                    ...
                                    <select id="service_id" name="service_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                        ...
                                        @foreach($services as $service)
                                            <option value="{{ $service->id }}" data-price="{{ $service->price }}" data-duration="{{ $service->duration }}" {{ $booking->service_id == $service->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <select id="staff_id" name="staff_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                        ...
                                        @foreach($staff as $member)
                                            <option value="{{ $member->id }}" {{ $booking->staff_id == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="booking_date" name="booking_date" type="date" class="mt-1 block w-full" :value="old('booking_date', $booking->booking_date->format('Y-m-d'))" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('booking_date')" />
                                </div>

                                <div>
                                    <x-input-label for="start_time" :value="__('Start Time')" />
                                    <x-text-input id="start_time" name="start_time" type="time" class="mt-1 block w-full" :value="old('start_time', $booking->start_time)" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('start_time')" />
                                </div>

                                <div>
                                    <x-input-label for="end_time" :value="__('End Time')" />
                                    <x-text-input id="end_time" name="end_time" type="time" class="mt-1 block w-full" :value="old('end_time', $booking->end_time)" required />
                        ...
                        <div class="mb-6">
                            ...
                            <div>
                                ...
                                <textarea id="notes" name="notes" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="3">{{ old('notes', $booking->notes) }}</textarea>
